...
const drawing = _ => {
    ...
    for (let i = 0; i < 5000; i++) {
        ...
        spiralsArr.forEach(elem => {
            ...
            elem.x = elem.x + 0.005
            elem.y = elem.x * Math.tan(elem.x)
            elem.r2 = Math.abs(Math.sin(elem.x * elem.x / 10)) * 1 / Math.abs(elem.x) * 1000;
            elem.alpha = Math.abs(Math.sin(elem.x)) / 10;
            // console.log('miki')
